...
public class Dapper<TContext> : IDapper<TContext>
    where TContext : IDbFactory
{
    private readonly TContext _context;

    public Dapper(TContext context)
    {
        _context = context;
    }

    public async Task<IEnumerable<T>> QueryAsync<T>(string query, Func<DbDataReader, T> map)
    {
    ...
    public async Task<IEnumerable<T>> QueryAsync<T>(string query, object? param, IDbTransaction? transaction)
    {
